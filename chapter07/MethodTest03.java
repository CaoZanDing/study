    ...
    注意：
        ...
        1.2、关于返回值类型：
            第一：返回值类型可以是任何类型，只要是java中合法的数据类型就行，数据类型
                包括基本数据类型和引用数据类型，也就是说返回值类型可以是：byte,short,
                int,long,float,double,boolean,char,String......

            第二：什么是返回值？
                返回值一般指的是一个方法执行结束之后的结果。
                结果通常是一个数据，所以被称为"值"，而且还叫"返回值"。
                方法就是为了完成某个特定的功能，方法结束之后大部分情况下都是
                有一个结果的，而体现结果的一般都是数据。数据得有类型。这就是返回值类型。
            ...
            第三：当一个方法执行结束不返回任何值的时候，返回值类型也不能空白，必须写上
                void关键字。所以void表示该方法执行结束后不返回任何结果。
            ...
            第五：只要有"return"关键字的语句执行，当前所在方法必然结束（不是整个程序结束）。
...
public class MethodTest03{
    ...
    public static void main(String[] args){
        ...
        //当一个方法有返回值的时候，可以选择接收，也可以选择不接收。
        //只不过这样没有意义，一般程序反悔了执行结果，都是需要接收这个结果的。
        //以下代码虽然没有使用变量接收这个返回值，但是这个值还是返回了。
        //返回之后内存马上释放，因为没有使用变量接收。
        MethodTest03.sum(200,300);
    }
