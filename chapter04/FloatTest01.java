/*
    关于java语言中的浮点型数据
        ...
        但是要注意的是，如果用在银行方面或者说用在财务方面，double也是远远不够的，在java中提供了
        一种精度更高的类型，这种类型专门使用在财务软件方面：java.math.BigDecimal(不是基本数据类型，
        属于引用数据类型。）

        float和double存储数据的时候都是存储的近似值。
        因为现实世界中有这种无限循环的数据。例如：3.3333333333333333....
